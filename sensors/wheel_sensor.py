...
class WheelSensor:
    def __init__(self, group_id: DevicesIds, sensor_id: SensorsIds, wheel_position: WheelPosition, init_wheel_state=0):
    ...
    def set_wheel_state(self, wheel_state):
        # sets new wheel_state
        self.wheel_state = wheel_state

    def run_wheel_simulation_process(self):
        # Creates wheel sensor subprocess daemon and runs it 
        try:
    ...
    def terminate_process(self):
        # terminates the wheel sensor process
        self.process.terminate()


    def _simulate_wheel_states(self):
        # Continuously reads data from the wheel pressure file and sends it to the canbus interface 
        try:
...
def _read_file_pressure_values() -> list[str]:
    # reads data from the pressure file
...
def simulate_sensors():
    ...
    wheel_sens1 = WheelSensor(
        sensor_id=SensorsIds.WHEEL_SENSOR_0, group_id=DevicesIds.VEHICLE_0, wheel_position=WheelPosition.FRONT_RIGHT)
    wheel_sens2 = WheelSensor(
        sensor_id=SensorsIds.WHEEL_SENSOR_1, group_id=DevicesIds.VEHICLE_0, wheel_position=WheelPosition.FRONT_LEFT)
    wheel_sens3 = WheelSensor(
        sensor_id=SensorsIds.WHEEL_SENSOR_2, group_id=DevicesIds.VEHICLE_0, wheel_position=WheelPosition.REAR_RIGHT)
    wheel_sens4 = WheelSensor(
        sensor_id=SensorsIds.WHEEL_SENSOR_3, group_id=DevicesIds.VEHICLE_0, wheel_position=WheelPosition.REAR_LEFT)
